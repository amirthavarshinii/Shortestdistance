...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		// I tried this since i wanted the vertices , i though i can take it from the map of adjacent list but then found it is private
	//	GraphAdjList obj = new GraphAdjList();
	//	for(Integer i : obj.map)
		ArrayList<Integer> l = new ArrayList<Integer>();
		// I tried this since i thought i could fetch the vertex key set from labels and integers but seems it is not populated yet 
	//	for( Integer i: vertexLabels.keySet()) {
		for(int i =0 ;i < this.getNumVertices(); i++) {
		l.add( getInNeighbors(i).size() + getNeighbors(i).size());
		}
		Collections.sort(l, Collections.reverseOrder(null));
